...
class IndexGroupCommand(Command, SessionCommandMixin):
    ...
    def pip_version_check(self, options, args):
        # type: (Values, List[Any]) -> Iterator[None]
        """
        Do the pip version check if not disabled.

        This overrides the default behavior of not doing the check.

        The check is deliberately split in two: everything that reads the
        environment or talks to an index happens *before* the command body
        runs, and afterwards only the already-computed prompt is rendered.
        Otherwise a command that just modified the environment pip runs from
        (``pip install``, most obviously) could get its freshly installed
        code executed by the check.
        """
        ...
        upgrade_prompt = None  # type: Optional[UpgradePrompt]
        try:
